...
df.ts = ts(df, frequency = 12, start=c(1962, 1))
plot.ts(df$milk_prod_per_cow_kg)
...
ggplot(EuStockDF,aes(x=Date, y=SMI))+geom_line()
  ...
  matrix_data <- reshape(data, idvar = "time", timevar = "time", direction = "wide")
  matrix_data <- matrix_data[, -1]
  
  # Creating a heatmap
  heatmap(matrix_data, 
          Colv = NA,  # Turn off column clustering
          scale = "column",  # Scale each column independently
          main = "Time Series Heatmap",  # Title of the plot
          xlab = "Time",  # X-axis label
          ylab = "Observations",  # Y-axis label
          col = colorRampPalette(c("blue", "white", "red"))(100)  # Custom color palette
  )
  

temp <-structure(list(date = c("2010-01-01", "2010-01-01", "2010-01-01", 
...
# now the same thing for my dataset

dat2$From <- as.POSIXct(dat2$From,format = "%Y-%m-%d %H:%M:%OS")
dat2$month <- month(dat2$From)
dat2$hour <- hour(dat2$From)
ggplot(dat2,aes(x=hour,y=month)) + geom_tile(aes(fill=PM10)) + geom_tile(aes(fill=PM2.5))

index <- which(is.na(dat2$PM10))
dat2$PM10[index] <- 0 
